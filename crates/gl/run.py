...
regFilename = "OpenGL-Registry/xml/gl.xml"
...
glextKHRplatformStrings = ["#include <KHR/khrplatform.h>", ""]
...
class WRCOutputGenerator(OutputGenerator):
    ...
    def makeCDecls(self, cmd):
        ...
        if n > 0:
            for i in range(0, n):
                ...
                parampass += params[i].find("name").text
                if i < n - 1:
                    paramdecl += ", "
                    parampass += ", "
        else:
        ...
        return (
            f"""__attribute__((import_name("{name}")))\n"""
            + f"""__attribute__((import_module("wr_gl")))\n"""
            + f"""{ret_type} imported_{name}{paramdecl};\n"""
            + "\n"
            + f"""{ret_type} {name}{paramdecl} """
            + "{\n"
            + f"""    imported_{name}{parampass};\n"""
            + "}\n\n"
        )
    ...
    def beginFile(self, genOpts):
        ...
        write("#include <GLES2/gl2.h>", file=self.outFile)
        self.newline()
        write("// clang-format off", file=self.outFile)
        self.newline()
        write("""__attribute__((import_name("present")))""", file=self.outFile)
        write("""__attribute__((import_module("wr_gl")))""", file=self.outFile)
        write("""void imported_wr_gl_present();""", file=self.outFile)
        self.newline()
        write(
            """void wr_gl_present() { imported_wr_gl_present(); }""", file=self.outFile
        )
...
def genHeaders():
    buildList = [  # GLES 2.0 API - GLES2/gl2.h (now with function pointers)
        (
            COutputGenerator(errFile=errWarn, warnFile=errWarn, diagFile=diag),
            CGeneratorOptions(
        ...
        (
            WRCOutputGenerator(errFile=errWarn, warnFile=errWarn, diagFile=diag),
            CGeneratorOptions(
                filename="../../example_apps/libs/wr_gl/wr_gl.c",
                apiname="gles2",
                profile="common",
                versions=gles2onlyPat,
                emitversions=allVersions,
